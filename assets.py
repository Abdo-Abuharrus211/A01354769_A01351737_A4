...
def make_character() -> dict:
    ...
    player = {"X-coordinate": 0, "Y-coordinate": 0, "Current HP": 7, "Current XP": 0, "Level": 1}
    return player
...
def describe_current_location(board: dict, character: dict, rows: int, columns: int) -> None:
    ...
    :precondition board: must be dictionary of the game space with coordinates as keys and integers as values
...
def game():
    ...
    while is_alive(character) and not achieved_goal:
        describe_current_location(board, character, rows, columns)
        ...
        if valid_move:
            ...
            print("Sorry you cannot move in that direction, please pick again")
    if achieved_goal:
